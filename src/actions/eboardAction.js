...
export const getArticles = (number, searchedItem) => dispatch => {
  dispatch(setEboardLoading);
  if(searchedItem.handle.length === 0 && searchedItem.title.length === 0 && searchedItem.content.length === 0) {
    axios.get(`/main/eboard/show/${number}`)
    .then(res => dispatch({
      type:GET_ARTICLES,
      payload: res.data
    }))
    ...
    axios.post(`/main/eboard/searched/show/${number}`, searchedItem)
    .then(res => dispatch({
      type: GET_ARTICLES,
      payload: res.data
    }))
...
export const submitComment = (id, comment) => dispatch => {
  axios.post(`/main/eboard/comment/${id}`, comment)
  .then(res => dispatch({
    type: GET_COMMENTS,
    payload: res.data
  }))
